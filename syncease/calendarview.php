...
$stmt = $pdo->prepare('SELECT Title, Category, StartDate, DueDate FROM tasks WHERE UserID = ?');
...
foreach ($tasks as $task) {
    ...
    $tasksByDate[$task['StartDate']][] = $task['Title'];
}
foreach ($tasks as $task) {
    echo '<div class="task">' . htmlspecialchars($task['Title']);
    echo '<br>Start: ' . $task['StartDate'];
    echo '<br>Due: ' . $task['DueDate'] . '</div>';
}
